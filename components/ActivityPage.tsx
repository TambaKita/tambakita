...
const ActivityPage: React.FC<ActivityPageProps> = ({ user }) => {
  ...
  // Ambil riwayat aktivitas DENGAN FILTER TANGGAL
  const fetchActivities = async () => {
    if (!user) return;
    
    try {
      setLoading(true);
      console.log('📥 Fetching activities with filter:', { filterDate, filterPondId });
      ...
      // Ambil data daily_activities dengan filter tanggal
      let query = supabase
      ...
      // Filter berdasarkan kolam jika dipilih
      if (filterPondId !== 'all') {
      ...
      if (activitiesError) {
        console.error('❌ Error activities:', activitiesError);
      ...
      // Buat map untuk profiles
      const profileMap = new Map(profiles?.map(p => [p.id, p.name]));

      // Buat map untuk ponds (hanya kolam yang user punya akses)
      ...
      const formattedLogs: ActivityLog[] = (activities || []).map((item: any) => ({
        ...
        pond_name: pondMap.get(item.pond_id) || 'Kolam Lain', // Ubah dari 'Unknown' jadi 'Kolam Lain'
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      setCatatanUmum(prev => prev ? `${prev} ${transcript}` : transcript);
    };
  ...
  // Load activities saat filter berubah
  useEffect(() => {
    if (ponds.length > 0) {
      fetchActivities();
    }
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="relative group">
            <textarea 
              ...
              className="w-full bg-white/15 border-none rounded-[1.8rem] p-5 text-sm focus:bg-white/25 outline-none font-bold h-24 placeholder-white/50 transition-all pr-14" 
              placeholder="Catatan tambahan (bisa pakai suara)..." 
            />
            <button 
              onClick={handleVoiceInput}
              className={`absolute right-4 bottom-4 w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                isListening ? 'bg-rose-500 animate-pulse text-white' : 'bg-white/20 text-white hover:bg-white/30'
              }`}
            >
              <i className={`fas ${isListening ? 'fa-microphone' : 'fa-microphone-lines'}`}></i>
            </button>
          </div>
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase text-slate-800">
                        {log.activity_type === 'WaterParameter' ? 'Parameter Air' : 
                         ...
                         log.activity_type === 'Medicine' ? 'Obat' : log.activity_type}
